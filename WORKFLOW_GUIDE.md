...
배포 보안 관리의 변경 항목(설정 수정, 개발본 동기화, 배포본 생성, 일괄 실행)은 작업 성공 직후 이 릴리즈 절차를 자동 호출합니다. 별도의 VS Code 커밋이나 `npm run version:release` 실행은 필요하지 않습니다. 읽기 전용 항목인 설정 확인, 자동 검증, 연장코드 생성은 자동 릴리즈하지 않습니다.
