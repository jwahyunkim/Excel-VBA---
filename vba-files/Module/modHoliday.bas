...
Public Sub EnsureConfigSheet()
    ...
    ws.Range(TASK_MAX_LENGTH_LABEL_CELL).Value = "내용 최대 글자 수"
    ...
    End If
    If Trim$(CStr(ws.Range(TASK_MAX_LENGTH_VALUE_CELL).Value)) = "" Then
        ws.Range(TASK_MAX_LENGTH_VALUE_CELL).Value = DEFAULT_TASK_MAX_LENGTH
    End If
    ...
    ws.Range(TASK_MAX_LENGTH_LABEL_CELL & ":" & TASK_MAX_LENGTH_VALUE_CELL).Borders.LineStyle = xlContinuous
    ...
    ws.Range(TASK_MAX_LENGTH_VALUE_CELL).NumberFormat = "0"
    ...
    Set rngTaskMaxLength = ws.Range(TASK_MAX_LENGTH_VALUE_CELL)
    ...
    rngTaskMaxLength.Validation.ErrorMessage = _
        "내용 최대 글자 수는 " & MIN_TASK_MAX_LENGTH & "~" & _
...
Public Function GetTaskMaxLength() As Long
    Dim ws As Worksheet
    ...
    settingValue = ws.Range(TASK_MAX_LENGTH_VALUE_CELL).Value
